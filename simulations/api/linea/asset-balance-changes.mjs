...
async function simulateTransaction() {
  ...
  try {
    console.time("Asset Changes: Quick Decoding Mode");
    ...
    console.timeEnd("Asset Changes: Quick Decoding Mode");
    console.log(simulationResponse.data);
    const assetChanges = simulationResponse.data.transaction.transaction_info.asset_changes;

    console.log("Asset Changes");
    console.log(assetChanges);

    const humanlyReadableAssetChanges = assetChanges.map(({ token_info, amount, dollar_value }) => {
      const { standard, name, symbol } = token_info;
      return `${standard} ${name} (${symbol?.toUpperCase()}) ${amount} wei ${dollar_value}`;
    });
    console.log("Humanly readable Asset Changes:");
    console.log(humanlyReadableAssetChanges);

    const balanceDiff = simulationResponse.data.transaction.transaction_info.balance_diff;

    console.log("Native Currency Balance Changes:");
